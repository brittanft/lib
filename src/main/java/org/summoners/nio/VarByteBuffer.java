...
/**
 * An expanding, no-capacity variable byte buffer wrapper with an output stream.
 * @author Joseph Robert Melsha (jrmelsha@olivet.edu)
 * @author Brittan Thomas
 */
public class VarByteBuffer extends OutputStream {
	 ...
	 *            if this byte buffer should be direct or not
	 */
	public VarByteBuffer(boolean direct) {
	 ...
	 */
	public VarByteBuffer(boolean direct, boolean little) {
	 ...
	 */
	public VarByteBuffer(boolean direct, boolean little, int initialSize) {
		this.direct = direct;
		this.little = little;
	...
	public void write(ByteBuffer buf) {
		if (buf.hasRemaining())
			ensureCapacity(buf.remaining()).put(buf);
	}
	 ...
	 */
	public ByteBuffer ensureCapacity(int capacity) {
		Validate.require(capacity >= 0);
		if (buffer != null) {
			...
			if (capacity > rem) {
				...
				capacity += size;
				capacity = Math.max(capacity, buffer.capacity() << 1);
				capacity = Math.max(capacity, initialSize);
				ByteBuffer b = BufferUtil.allocate(capacity, direct, little);
				b.put(buffer);
				buffer = b;
			}
		} else {
			capacity = Math.max(capacity, initialSize);
			buffer = BufferUtil.allocate(capacity, direct, little);
		}
		return buffer;
	}
